...
function MetricCounter({ index, reduceMotion }: { index: number; reduceMotion: boolean }) {
  ...
  return <strong ref={ref}>{prefix}{value.toLocaleString('fa-IR')}{suffix}</strong>
}
          ...
          <div className={styles.metrics}>
            {impactMetrics.map((metric, index) => (
              <motion.div key={metric.label} initial={reduceMotion ? false : { opacity: 0, y: 38 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: reduceMotion ? 0 : 0.65, delay: reduceMotion ? 0 : index * 0.16, ease: [0.22, 1, 0.36, 1] }}>
                <MetricCounter index={index} reduceMotion={Boolean(reduceMotion)} /><span>{metric.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className={styles.marqueeTrack}>
            {[0, 1].map((copy) => (
              <div className={styles.marqueeGroup} key={copy} aria-hidden={copy === 1 ? true : undefined}>
                {stationedTeams.map((team) => <span key={team.name}><i aria-hidden="true">✦</i>{team.name}</span>)}
              </div>
            ))}
          </div>
        </div>
